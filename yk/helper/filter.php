 ...
 *
 *      $Id: yk_assist_filter.php 2013-8-29$
 */
...
class filter{
	...
	public static function verify($value, $methodname, $extra = null){
		...
		$status = false;
		if ($methodname) {
			if (is_string($methodname) && strpos($methodname, '|')) {
				list($main_ck, $helper_ck) = explode('|', $methodname);
				$main_ck = 'ck_'.$main_ck;
				$helper_ck = 'ck_'.$helper_ck;
				if (method_exists('yk\\helper\\filter', $main_ck) && method_exists('yk\\helper\\filter', $helper_ck)) {
					if (self::$main_ck($value, $extra, $helper_ck)) {
						return true;
					}
				}else{
					throw new \yk\exception\helper('filter', "$methodname not find");
				}
			}else{
				...
				foreach ($methodname as $method){
					$method = 'ck_'.$method;
					if (method_exists('yk\\helper\\filter', $method)) {
						if (self::$method($value, $extra)) {
							return true;
						}
					}else{
						throw new \yk\exception\helper('filter', "$method not find");
					}
				}
			}
		}
	...
	protected static function ck_phone($value){
		return preg_match('/^[01]{1}([0-9]){3,19}$/', $value);
	}
	...
	protected static function ck_idcard($value){
		...
		switch($len){
			...
			case 18:
				$warr = array(7,9,10,5,8,4,2,1,6,3,7,9,10,5,8,4,2,1);
				$aarr = array('1','0','x','9','8','7','6','5','4','3','2');
				$sum = 0;
				for($i=0; $i<17; $i++) {
					$sum += $value[$i] * $warr[$i];
				}
